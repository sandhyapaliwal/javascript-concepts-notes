...
2. **When would you use flatMap() instead of map()?**
   - When you need to map AND flatten the result in one operation

3. **What does Object.freeze() do?**
   - Prevents any modifications to an object (properties cannot be added, deleted, or modified)

4. **Difference between parseInt and parseFloat?**
   - `parseInt()` returns an integer, `parseFloat()` returns a decimal number
